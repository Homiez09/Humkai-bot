...
module.exports = {
    ...
    run: async (interaction, client) => {
        ...
        try {
            ...
            if (!channelData) return interaction.reply('กรุณาติดตั้งก่อนนะครับ /install wordle-game');
            if (channelData.wordle_ID !== interaction.channel.id) return interaction.reply(`ไปเล่นที่ห้องนี้ <#${channelData.wordle_ID}>`);
        } catch (err) {
            console.log(err);
        }
        ...
        if (wordleData.working) return interaction.reply(`เกมกำลังดำเนินการอยู่ ${wordleData.doing.length}`);
