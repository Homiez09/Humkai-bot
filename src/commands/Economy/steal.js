...
module.exports = {
  ...
  run: async (interaction, client, profileData) => {
    ...
    try {
        ...
        return interaction.reply({
          embeds: [
            await new MessageEmbed()
              .setDescription(':x: | ไม่ปล้นเงินตัวเองได้!')
              .setColor('RED'),
          ],
          ephemeral: false,
        });
      ...
      if (!(targetChk.coins > 1000))
        return interaction.reply({
          embeds: [
            await new MessageEmbed()
              .setDescription(':x: | เป้าหมายมีเงินไม่พอ')
              .setColor('RED'),
          ],
          ephemeral: false,
        });
      if (targetChk.protection)
        return interaction.reply({
          embeds: [
            await new MessageEmbed()
              .setDescription(':x: | ผู้ใช้งานนี้ถูกปล้นไปแล้ว')
              .setColor('RED'),
          ],
        });
      ...
      target.send({
        embeds: [
          await new MessageEmbed()
            .setDescription(
              `คุณถูกปล้นเงินไป ${money}${process.env.CURRENCY} โดยใครบางคน และคุณจะได้รับการป้องกันจนกว่าคุณจะพิมพ์คำสั่งอีกครั้ง`,
            )
            .setColor('RED'),
        ],
        ephemeral: false,
