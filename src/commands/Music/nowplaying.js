...
module.exports = {
  ...
  run: async (interaction, client) => {
    ...
    console.log(
      chalk.magenta(
        `[COMMAND] NowPlaying used by ${interaction.user.tag} from ${interaction.guild.name}`,
      ),
    );
    ...
    const embeded = new MessageEmbed()
      ...
      .addField(
        'Download:',
        `**[Click Here](https://www.mp3fromlink.com/watch?v=${song.identifier})**`,
        true,
      )
      .addField(
    ...
    if (realtime === 'true') {
      interval = setInterval(async () => {
        ...
        if (NEmbed)
          NEmbed.edit({ content: ' ', embeds: [embeded], components: [row] });
      }, 5000);
    ...
    collector.on('collect', async (interaction) => {
      ...
      if (id === 'pause') {
        ...
        embeded.fields[6] = {
          name: `Current Duration: \`[${formatDuration(
            player.position,
